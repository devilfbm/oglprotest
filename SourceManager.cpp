...
void SourceManager::Add(const char * path)
{
	...
	if (LoadOBJ(path, out_vertices, out_uvs, out_normals))
	{
		...
		out_vertices_list.push_back(out_vertices);
	}
}

bool SourceManager::LoadOBJ(
	const char * path,
	std::vector<glm::vec3> & out_vertices,
	std::vector<glm::vec2> & out_uvs,
	std::vector<glm::vec3> & out_normals
	)
{
	...
	//bool res = loadOBJ("D://Homework//–Èƒ‚œ÷ µ//SimpleReconstruction//untitled.obj", vertices, uvs, normals);
